...
from cqed_sim.pulses.builders import build_rotation_pulse, build_sqr_multitone_pulse
...
INSPECTED_FILES = [
    ...
    str(SIMULATION_ROOT / "cqed_sim" / "pulses" / "builders.py"),
    ...
    str(SIMULATION_ROOT / "examples" / "workflows" / "sequential" / "pulse_unitary.py"),
    str(SIMULATION_ROOT / "examples" / "studies" / "snap_opt" / "model.py"),
    ...
    for phase_sign in (+1, -1):
        for omega_sign in (+1, -1):
            ...
            row = {
                ...
                "process_fidelity_vs_experiment": float(fidelity),
            }
            rotation_rows.append(row)
            sqr_rows.append(dict(row))
    best_rotation = max(rotation_rows, key=lambda row: row["process_fidelity_vs_experiment"])
    best_sqr = max(sqr_rows, key=lambda row: row["process_fidelity_vs_experiment"])
    return {
        "rotation_rows": rotation_rows,
        "sqr_rows": sqr_rows,
        "best_rotation_match": best_rotation,
        "best_sqr_match": best_sqr,
        "experiment_waveform_formula": "w(t) = coeff * w0(t) * exp(+i phi_eff) * exp(+i omega t)",
    ...
    return [
        {
            ...
            "proving_test": "test_bloch_y_matches_sigma_y_expectation; test_fock_weighted_bloch_contributions_sum_to_joint_trace; test_rotation_benchmarks_capture_standard_and_historical_y_signs",
        },
    ]
...
def run_full_audit(config: AuditConfig = AuditConfig()) -> dict[str, Any]:
    ...
    return {
        ...
        "success_criteria_answer": {
            ...
            "minimal_patch": "completed; only explicitly named historical audit helpers remain for legacy qubox comparison",
        },
