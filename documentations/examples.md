...
| `custom_gate_primitives_demo.py` | Registered custom callable primitive fitted to `Rx(pi/2)` with an excited-state population validation plot | `tutorials/30_advanced_protocols/03_unitary_synthesis_workflow.ipynb` |
| `unitary_synthesis_optimization_catalog.py` | Validated catalog of duration-, state-, reduced-state-, channel-, isometry-, observable-, trajectory-, robustness-, leakage-, Pareto-, warm-start-, and gate-order synthesis workflows | `tutorials/30_advanced_protocols/03_unitary_synthesis_workflow.ipynb` |
...
| `callable_optimal_control_demo.py` | Callable waveform ansatz optimized through the standard GRAPE pipeline using named amplitude and phase parameters | `tutorials/optimal_control.md` |
| `optimal_control_gate_time_sweep_demo.py` | Parallel outer-loop gate-time sweep that records duration versus objective and fidelity | `tutorials/optimal_control.md` |
| `structured_to_grape_refinement_demo.py` | Structured Gaussian-DRAG warm start lifted into piecewise GRAPE for final local refinement | `tutorials/optimal_control.md` |
