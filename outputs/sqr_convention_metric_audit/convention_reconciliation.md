...
## Derived Mapping
- Positive `phi_eff` rotates the IQ phasor counterclockwise in the complex plane.
- With `H_drive = Re[w]*sigma_x/2 + Im[w]*sigma_y/2`, axis phase is `phi_axis = arg(w) = phi_eff`.
- Therefore `R_xy(theta, phi_axis)` uses the same numeric phase in both the lab waveform and the canonical simulator builders.
