...
from cqed_sim.analysis.experiment_convention_audit import (
...
def test_legacy_waveform_phase_mapping_matches_simulator_sign_scan() -> None:
    ...
    assert sign_scan["best_rotation_match"]["process_fidelity_vs_legacy"] > 0.999999
    assert sign_scan["best_sqr_match"]["phase_sign"] == 1
    assert sign_scan["best_sqr_match"]["omega_sign"] == 1
    assert sign_scan["best_sqr_match"]["process_fidelity_vs_legacy"] > 0.999999
