...
Runtime backend settings that build a `SimulationConfig` may include `nsteps` and `solver_options`; those fields are forwarded to QuTiP `propagator(...)` when model-backed waveform primitives are replayed.
