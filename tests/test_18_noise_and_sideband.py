...
def test_qubit_t1_decay_matches_exponential():
    ...
    r = simulate_sequence(m, c, m.basis_state( 1,0), {}, SimulationConfig(), noise=NoiseSpec(t1=t1))
...
def test_cavity_ringdown_matches_kappa():
    ...
    psi = qt.tensor( qt.basis(m.n_tr, 0),qt.coherent(m.n_cav, alpha))
    c = SequenceCompiler(dt=0.05).compile([], t_end=8.0)
...
def test_trace_preserved_and_positivity_bounded_open_system():
    ...
    psi = (m.basis_state( 0,1) + 1j * m.basis_state( 1,2)).unit()
...
def test_sideband_swap_one_photon_pi_pulse():
    ...
    r = simulate_sequence(m, c, m.basis_state( 1,0), {"sb": "sideband"}, SimulationConfig(frame=FrameSpec()))
    p_g1 = abs(m.basis_state( 0,1).overlap(r.final_state)) ** 2
    p_e0 = abs(m.basis_state( 1,0).overlap(r.final_state)) ** 2
    assert p_g1 > 0.98
    assert p_e0 < 0.02
...
def test_sideband_oscillation_frequency_matches_g():
    ...
    r = simulate_sequence(m, c, m.basis_state( 1,0), {"sb": "sideband"}, SimulationConfig(store_states=True))
    pg1 = np.array([abs(m.basis_state( 0,1).overlap(s)) ** 2 for s in r.states], dtype=float)
...
def test_sideband_selectivity_breaks_with_detuning():
    ...
    c = SequenceCompiler(dt=0.01).compile([p], t_end=t_pi)
    rr = simulate_sequence(m_res, c, m_res.basis_state( 1,0), {"sb": "sideband"}, SimulationConfig())
    rd = simulate_sequence(m_det, c, m_det.basis_state( 1,0), {"sb": "sideband"}, SimulationConfig())
    p_res = abs(m_res.basis_state( 0,1).overlap(rr.final_state)) ** 2
    p_det = abs(m_det.basis_state( 0,1).overlap(rd.final_state)) ** 2
