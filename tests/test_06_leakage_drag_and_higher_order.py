...
def test_multilevel_leakage_and_drag_directionality():
    ...
    proj_f = qt.tensor(qt.qeye(model.n_cav), qt.basis(model.n_tr, 2) * qt.basis(model.n_tr, 2).dag())
...
def test_higher_order_terms_smoke():
    ...
    def rel_phase(model, n, q_level):
        psi = (model.basis_state(n, 0) + model.basis_state(n, q_level)).unit()
        rho = qt.ptrace(
            simulate_sequence(model, compiled, psi, {}, SimulationConfig(frame=FrameSpec())).final_state, 1
        )
        return np.angle(rho[0, q_level])

    p1_lin = rel_phase(model_linear, 1, 1)
    p2_lin = rel_phase(model_linear, 1, 2)
    p1_hi = rel_phase(model_high, 1, 1)
    p2_hi = rel_phase(model_high, 1, 2)
