...
def _test_identity_target_gives_high_fidelity(base_config: dict[str, Any]) -> None:
    config = _benchmark_config(base_config)
    config.update({"allow_zero_theta_corrections": False})
    target = _identity_target()
    result = calibrate_guarded_sqr_target(target, config, lambda_guard=0.1)
    direct = evaluate_guarded_sqr_target(target, config, corrections={}, lambda_guard=0.1)
    if result.calibration.levels:
        raise AssertionError("Identity target should not create per-manifold optimization levels when zero-theta levels are disabled.")
    if result.metadata.get("active_levels") != []:
        raise AssertionError(f"Expected no active levels for identity target, got {result.metadata.get('active_levels')}.")
    if not np.isclose(result.logical_fidelity, float(direct["logical_fidelity"]), atol=1.0e-12):
        raise AssertionError("Identity target shortcut changed the logical fidelity relative to direct drift-only evaluation.")
    if not np.isclose(result.epsilon_guard, float(direct["epsilon_guard"]), atol=1.0e-12):
        raise AssertionError("Identity target shortcut changed the guard leakage relative to direct drift-only evaluation.")
...
def run_sqr_calibration_sanity_suite(base_config: dict[str, Any]) -> list[dict[str, str]]:
    tests = [
        ...
        ("Benchmark Test 4: identity target uses drift-only shortcut", lambda: _test_identity_target_gives_high_fidelity(base_config)),
...
def test_direct_fock_frequencies_override_chi_polynomial_in_multitone_simulation():
    config = _benchmark_config({"cavity_fock_cutoff": 3})
    config.update(
        {
            "omega_q_hz": 6.1503587644830475e9,
            "fock_fqs_hz": [6.1473587644830475e9, 6.1443587644830475e9, 6.1413587644830475e9, 6.1383587644830475e9],
        }
    )
    target = RandomSQRTarget(
        target_id="fock_override_target",
        target_class="sparse",
        logical_n=2,
        guard_levels=2,
        theta=(np.pi / 2.0, 0.0, 0.0, 0.0),
        phi=(0.0, 0.0, 0.0, 0.0),
    )
    prepared = _build_multitone_simulation(target, config, corrections={})
    tone0 = prepared["tone_specs"][0]
    expected_waveform_hz = -(config["fock_fqs_hz"][0] - config["omega_q_hz"])
    if not np.isclose(tone0["omega_rad_s"] / (2.0 * np.pi), expected_waveform_hz):
        raise AssertionError(
            f"Direct fock frequency override not applied. expected={expected_waveform_hz}, actual={tone0['omega_rad_s'] / (2.0 * np.pi)}"
        )
