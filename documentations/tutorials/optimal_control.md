...
### Callable waveform ansatz

Code path: `examples/callable_optimal_control_demo.py`

Artifacts generated by the example:

- `outputs/callable_optimal_control_demo/result.json`
- `outputs/callable_optimal_control_demo/waveforms.csv`
- `outputs/callable_optimal_control_demo/summary.json`

The example uses `CallableParameterization` with two named optimizer variables, `amplitude` and `phase`, to map a compact ansatz directly onto two control quadratures. The evaluator signature is:

```python
evaluator(values, time_grid, control_terms) -> waveform
```

with an optional analytic pullback:

```python
pullback_evaluator(gradient_command, values, time_grid, control_terms, waveform)
```

Measured output from the checked-in run:

| Quantity | Value |
| --- | --- |
| Success | `True` |
| Objective value | `0.0` |
| Nominal fidelity | `1.0` |
| Optimized amplitude | `3.1415926542139463` |
| Optimized phase | `0.7000000000000006` |

This example is deliberately small, but it shows the important interface contract: arbitrary low-dimensional waveform maps can plug into the same GRAPE, penalty, hardware, pulse-export, and simulator-replay pipeline as the built-in parameterizations.

### Gate-time optimization by duration sweep

Code path: `examples/optimal_control_gate_time_sweep_demo.py`

Artifacts generated by the example:

- `outputs/optimal_control_gate_time_sweep_demo/best_result.json`
- `outputs/optimal_control_gate_time_sweep_demo/duration_sweep.csv`
- `outputs/optimal_control_gate_time_sweep_demo/summary.json`

The current public gate-time workflow is an explicit outer duration sweep, not a differentiable free-final-time inner solver. Each candidate duration rebuilds the scaled time grid, re-solves the control problem, and records the best result in a `GateTimeOptimizationResult`.

The checked-in example ran with `GateTimeOptimizationConfig(max_workers=os.cpu_count(), warm_start_strategy="none")`, which used `32` CPU worker threads on the validation machine. If you want the inner solves on GPU, pair the outer loop with `GrapeConfig(engine="jax", jax_device="gpu")`.

Measured duration scan from `outputs/optimal_control_gate_time_sweep_demo/duration_sweep.csv`:

| Duration (s) | Objective | Nominal fidelity |
| --- | --- | --- |
| `2.0` | `0.2919265817264288` | `0.7080734182735712` |
| `2.6` | `0.07155562331552634` | `0.9284443766844737` |
| `3.2` | `0.0` | `1.0` |
| `4.0` | `0.0` | `1.0` |

In this toy constrained-amplitude example, short durations are infeasible, while `3.2 s` is the shortest candidate that reaches unit fidelity.

### Structured pulse warm start followed by GRAPE refinement

Code path: `examples/structured_to_grape_refinement_demo.py`

Artifacts generated by the example:

- `outputs/structured_to_grape_refinement_demo/structured_result.json`
- `outputs/structured_to_grape_refinement_demo/grape_result.json`
- `outputs/structured_to_grape_refinement_demo/summary.json`

This workflow uses a model-backed transmon example. The structured stage first optimizes a Gaussian-DRAG ansatz. The command waveform from that result is then lifted into a piecewise-constant GRAPE problem and refined locally with `solve_structured_then_grape(...)`.

Measured before/after summary from `outputs/structured_to_grape_refinement_demo/summary.json`:

| Stage | Objective | Nominal fidelity |
| --- | --- | --- |
| Structured warm start | `2.6700124807765846e-06` | `0.9999973299875192` |
| GRAPE refinement | `4.3876013933186186e-13` | `0.9999999999995612` |

The objective improvement recorded by the example run is `2.6700120420164453e-06`. This is the intended use case for the hybrid workflow: keep a physically interpretable low-dimensional pulse family for the coarse search, then hand the resulting command waveform to GRAPE for the final local cleanup.
...
Choose `CallableParameterization` when:

- the waveform is naturally described by a small analytic parameter vector,
- the built-in rectangular or linear-basis parameterizations are not the right abstraction,
- you want to keep the standard solver/objective/hardware pipeline while swapping only the parameter map.

Choose gate-time optimization helpers when:

- the total duration is itself a design variable,
- amplitude or bandwidth limits make some durations infeasible,
- you want a transparent duration-versus-fidelity table rather than a hidden free-final-time optimizer.

Choose structured-to-GRAPE refinement when:

- you want a hardware-meaningful ansatz for the coarse search,
- the final local cleanup still benefits from slice-level freedom,
- you want a warm start that stays on the same `ControlProblem` conventions.
