...
def test_chi_projector_vs_pauli_equivalence_with_offset():
    ...
    n_c = ops["n_c"]
    n_q = ops["n_q"]
    i_tot = qt.tensor(qt.qeye(n_cav), qt.qeye(2))
    ...
    psi0 = (model.basis_state(1, 0) + model.basis_state(1, 1)).unit()
...
def test_chi_sign_matches_pull_direction():
    ...
    for t in times:
        compiled = SequenceCompiler(dt=0.1).compile([], t_end=t)
        p0 = (model.basis_state(0, 0) + model.basis_state(0, 1)).unit()
        p1 = (model.basis_state(1, 0) + model.basis_state(1, 1)).unit()
        ...
        phases0.append(-np.angle(qt.ptrace(r0.final_state, 1)[0, 1]))
        phases1.append(-np.angle(qt.ptrace(r1.final_state, 1)[0, 1]))
