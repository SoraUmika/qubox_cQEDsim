...
class ControlSystem:
    ...
    def __post_init__(self) -> None:
        ...
        collapse_ops = tuple(
            as_square_matrix(operator, name=f"ControlSystem[{self.label}].collapse_operator")
        ...
        for operator in collapse_ops:
            if operator.shape != drift.shape:
                raise ValueError("All collapse operators must have the same shape as the drift Hamiltonian.")
        ...
        object.__setattr__(self, "collapse_operators", collapse_ops)
...
def build_control_system_from_model(
    ...
    *,
    ...
    collapse_ops = tuple(
        ...
        for operator in runtime_collapse_operators(model, noise)
    )
    ...
    for member in ensemble_members:
        systems.append(
            build_control_system_from_model(
                ...
                frame=frame,
                channel_specs=channel_specs,
                noise=member.noise if member.noise is not None else noise,
